...
def init():
    ...
    return (face_detector, open_eyes_detector, left_eye_detector,right_eye_detector, video_capture, images) 
...
def detect_and_display(model, video_capture, face_detector, open_eyes_detector, left_eye_detector, right_eye_detector, data, eyes_detected):
    ...
    #success stores True/False, if getting frame is sucess then True.
    ...
    for (x,y,w,h) in faces:
        ...
        ##CHANGING THE CODE BELOW PROVIDES AUTHENTICATION
        ##IMPORTANT : CHECK THE COMMENTS FOR THE CALLER
        if isBlinking(eyes_detected[name],3):
            cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
            # Display name OR AUTHENTICATE
            y = y - 15 if y - 15 > 15 else y + 15
            cv2.putText(frame, name, (x, y), cv2.FONT_HERSHEY_SIMPLEX,0.75, (0, 255, 0), 2)
            print("The Eye has blinked. The person's name is",name)
    return frame


if __name__ == "__main__":
    (face_detector, open_eyes_detector,left_eye_detector,right_eye_detector, video_capture, images) = init()
    ...
    while True:        
        ...
        ##REMOVE THE LINE BELOW WITH SOME OTHER CONTROL IF AUTHENTICATION IS NEEDED
